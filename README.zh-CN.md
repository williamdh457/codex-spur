...
```text
~/Library/Application Support/com.codexspur.desktop/   # macOS 本地库
%APPDATA%\com.codexspur.desktop\                      # Windows 本地库
```
...
- **macOS Apple Silicon**（`aarch64` DMG）和/或 **Windows x64**（NSIS 安装包 `.exe`）  
- 已安装 ChatGPT Desktop / Codex，并读取用户 Codex 目录：  
  - macOS：`~/.codex`  
...
1. 打开 [Releases](https://github.com/williamdh457/codex-spur/releases/latest) 下载 DMG（例如 `Codex.Spur_0.1.11_aarch64.dmg`） 
...
1. 在同一 Release 下载 `Codex.Spur_<version>_x64-setup.exe`  
2. 运行 NSIS 安装程序（默认当前用户安装）  
3. 若 **SmartScreen** 提示未知发布者：选 **更多信息 → 仍要运行**（v1 未签名，与未公证 mac DMG 策略一致）  
4. 启动 **Codex Spur**，配置供应商后 **Review & Apply**，写入 `%USERPROFILE%\.codex`  
5. 使用第三方模型时保持托盘进程在线  

Windows 依赖本机已安装且可读 `%USERPROFILE%\.codex` 的 ChatGPT / Codex Desktop；自定义 `model_providers` 表现可能与 macOS 不完全一致。
...
npm run bundle:nsis     # Windows NSIS（需在 Windows 或 CI 上构建）
```
