...
- **CC Switch–aligned Chat Completions vs Responses routing**: Codex always hits Spur `/v1/responses`. **Chat bridge** (Responses↔Chat Completions, JSON + SSE tool_calls): Kimi, MiniMax, OpenCode Go, custom OpenAI-compatible (unless protocol explicitly says Responses), and legacy DeepSeek chat ids. **Responses passthrough**: OpenAI, xAI/Grok, DeepSeek V4 Flash/Pro. MiniMax/custom kind defaults updated to `Chat Completions` to match CCS openai_chat presets. No extra product policies (no failure circuit-breakers).
